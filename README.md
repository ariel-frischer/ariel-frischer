<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,100:70a5fd&height=160&section=header&text=Ariel%20Frischer&fontSize=42&fontColor=ffffff&fontAlignY=35&animation=fadeIn&desc=Building%20Agentic%20Coding%20Tools&descSize=18&descColor=a9b1d6&descAlignY=55" width="100%" />

## 🧑‍💻 About Me

Software Engineer in California building **agentic coding productivity tools**.

🚀 Founder of [RepoBird.ai](https://repobird.ai) & [Rentalot.ai](https://rentalot.ai) · 💚 Neovim enthusiast · 🔧 Open-source contributor

## ⭐ Featured Projects

| Project | Description | |
|---------|-------------|---|
| [autospec](https://github.com/ariel-frischer/autospec) | 📋 CLI for streamlined spec-driven development | ![stars](https://img.shields.io/github/stars/ariel-frischer/autospec?style=social) |
| [bmessages.nvim](https://github.com/ariel-frischer/bmessages.nvim) | 💬 Better `:messages` window for Neovim with auto-updating buffer | ![stars](https://img.shields.io/github/stars/ariel-frischer/bmessages.nvim?style=social) |
| [claude-clean](https://github.com/ariel-frischer/claude-clean) | ✨ Beautiful terminal parser for Claude Code's streaming JSON output | ![stars](https://img.shields.io/github/stars/ariel-frischer/claude-clean?style=social) |
| [alias-gen](https://github.com/ariel-frischer/alias-gen) | ⚡ Automate shell alias creation from your command history | ![stars](https://img.shields.io/github/stars/ariel-frischer/alias-gen?style=social) |
| [chlog](https://github.com/ariel-frischer/chlog) | 📝 CLI for YAML-first changelog management | ![stars](https://img.shields.io/github/stars/ariel-frischer/chlog?style=social) |
| [kb](https://github.com/ariel-frischer/kb) | 🧠 CLI RAG tool — index 30+ doc formats, hybrid search, sourced answers | ![stars](https://img.shields.io/github/stars/ariel-frischer/kb?style=social) |

## 🛠️ Tech Stack

**Languages**

[![Languages](https://skillicons.dev/icons?i=go,python,ts,lua,rust&perline=8)](https://skillicons.dev)

**Frameworks & Libraries**

[![Frameworks](https://skillicons.dev/icons?i=nextjs,react,tailwind,threejs&perline=8)](https://skillicons.dev)

**Infrastructure & Tools**

[![Infra](https://skillicons.dev/icons?i=docker,aws,terraform,postgres,sqlite,neovim,linux,githubactions&perline=8)](https://skillicons.dev)

## 📊 Stats

<p>
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=ariel-frischer&theme=nord_dark" width="100%" />
</p>
<p>
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=ariel-frischer&theme=nord_dark" width="32%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=ariel-frischer&theme=nord_dark" width="32%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=ariel-frischer&theme=nord_dark&utcOffset=-7" width="32%" />
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,100:70a5fd&height=80&section=footer" width="100%" />
